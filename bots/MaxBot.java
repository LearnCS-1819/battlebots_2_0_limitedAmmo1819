...
/**
 * The MaxBot is a bot which intelligently targets other bots and avoids incoming
 ...
 * @version v1.0
 */
public class MaxBot extends Bot {
    ...
    public int getMove(BotInfo me, boolean shotOK, BotInfo[] liveBots, BotInfo[] deadbots, Bullet[] bullets) {
        ...
        if ((me.getBulletsLeft() < 5) && (deadbots != null)) {
            ...
            // check to see if dead bot is in range (to the right or left)
            if (Math.abs(me.getY() - deadbot.getY()) < (Bot.RADIUS*2)) {
                ...
                // move appropriately towards the dead bot
            ...
            // determine most efficient way of travelling towards the nearest dead bot and pursue it
            if (xDist2 < yDist2) {
        ...
        if (liveBots.length > 0 && bullets.length > 0) {
            ...
            } else if (bulletDistance >= AVOIDANCE_TRIGGER) {
...
//                        if (me.getY() + Bot.RADIUS >= closest.getY() && me.getY() + Bot.RADIUS <= closest.getY() + (Bot.RADIUS * 2)) {
